...
ticker1 = ['SPX Index']
ticker2 = ['SPW Index']
fields = ['PX_OPEN', 'PX_HIGH', 'PX_LOW', 'PX_LAST', 'MOV_AVG_50D', 'MOV_AVG_20D', 'MOV_AVG_5D']
dataPeriod = "WEEKLY"
startDate = dt(2003,5,2)
endDate = dt.today()
...
five1= df1['MOV_AVG_5D'].tolist()
twenty1 = df1['MOV_AVG_20D'].tolist()
fifty1 = df1['MOV_AVG_50D'].tolist()
...
five2= df2['MOV_AVG_5D'].tolist()
twenty2 = df2['MOV_AVG_20D'].tolist()
fifty2 = df2['MOV_AVG_50D'].tolist()

buySignalIndexPrelim1 = []
sellSignalIndexPrelim1 = []

for i in range(1,len(dates1)):
    if five1[i] > twenty1[i] and twenty1[i] > fifty1[i] and twenty1[i-1] < fifty1[i-1]:
...
for i in range(1,len(dates2)):
    if five2[i] > twenty2[i] and twenty2[i] > fifty2[i] and twenty2[i-1] < fifty2[i-1]:
...
if buySignalIndex1[-1] != buySignalIndexPrelim1[-1]:
    add = buySignalIndexPrelim1[len(buySignalIndex1):][0]
...
if buySignalIndex2[-1] != buySignalIndexPrelim2[-1]:
    add = buySignalIndexPrelim2[len(buySignalIndex2):][0]
...
if sellSignalIndex1[-1] != sellSignalIndexPrelim1[-1]:#
    add = sellSignalIndexPrelim1[len(sellSignalIndex1):][0]
...
if sellSignalIndex2[-1] != sellSignalIndexPrelim2[-1]:
    add = sellSignalIndexPrelim2[len(sellSignalIndex2):][0]
