...
def fitnetFunction(inputs, targets, dset, hidSize):
    ...
    MLP = MLPRegressor(hidden_layer_sizes=([15 for i in range(0,hidSize)]), max_iter=2000)
    MLP.fit(X_train, y_train.values.ravel())
    
    final = scaler.transform(dset)
